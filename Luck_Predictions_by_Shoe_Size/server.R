...
shinyServer(function(input, output) {
    DataInput <- reactive({
        genderNum <- switch(input$gender,
                             "male" = 1,
                             "female" = 2,
                             "other" = 3)
        x <- genderNum * input$number - input$shoe * 2
        AnsNum <- ifelse(x>0, x, x+100)
    })
    listAns <-list("A beautiful, smart, and loving person will be coming into your life.",
    ...
    output$outAns <- renderText({
        Ans <-listAns[[DataInput()]]
    })    
    #output$aboutApp <- renderText({"text})
})
